...
def send_msg_via_slack(data, header=None):
    ...
    for d in data:
        ...
        blocks.append(block)
    logger.debug(f"Sending blocks: {blocks}")
    res = send_msg_via_slack_raw(cfg['slack']['webhook_url'], {'blocks': blocks})
    if res.status_code != 200:
        logger.error(res.text)
    else:
        logger.debug(f"Slack message {data} sent")
        

# --- MAIN EXECUTION ---
if __name__ == "__main__":
    # --- Load config and setup DB ---
    send_msg_via_slack("Eikon News job started")
    ...
    send_msg_via_slack("Eikon News job finished")
