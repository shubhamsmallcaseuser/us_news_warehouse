    ...
    for start_date, end_date in date_chunks:
        ...
        while attempt < max_retries:
            try:
                ...
                if not df.empty:
                    if len(df) == 100:
                        global CAP_HIT_COUNT
                        CAP_HIT_COUNT += 1
                        logger.warning(f"[CAP HIT] {instrument}: exactly 100 headlines returned — "
                                       f"window may be truncated ({start_str} to {end_str})")
                    df['RIC'] = instrument
                    all_news.append(df)
                break
                ...
                if ("400 bad request" in error_str or "backend error" in error_str or "not defined" in error_str):
                    ...
                    global SKIP_COUNT
                    SKIP_COUNT += 1
                    logger.warning(f"[SKIP] {instrument}: non-retryable error, skipping RIC. Error: {e}")
                    break
    if all_news:
...
# ---------------------------------------------------------------------------
# SLACK ALERTS  (mirrors news_headline_eikon_fetch.py exactly)
# ---------------------------------------------------------------------------
def send_msg_via_slack_raw(url, payload):
    res = requests.post(url, data=json.dumps(payload))
    return res

def send_msg_via_slack(data, header=None):
    try:
        cfg = load_config()
        blocks = []
        if header:
        ...
        for d in data:
            if isinstance(d, pd.DataFrame):
                d = d.to_markdown(index=False, tablefmt="github", headers="keys")
                d = f"```\n{d}\n```"
            elif not isinstance(d, str):
                raise TypeError(f"Unsupported data type: {type(d)}, Currently supporting pd.DataFrame and str")
            blocks.append({
                "type": "section",
                "text": {"type": "mrkdwn", "text": d}
            })
        logger.debug(f"Sending Slack blocks: {blocks}")
        res = send_msg_via_slack_raw(cfg['slack']['webhook_url'], {'blocks': blocks})
        if res.status_code != 200:
            logger.error(f"Slack send failed: {res.text}")
        else:
            logger.debug(f"Slack message sent: {data}")
    except Exception as slack_err:
        logger.warning(f"Slack notification failed (job continues): {slack_err}")
